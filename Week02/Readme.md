...
單靠一種網路模式無法同時滿足「連接外網」和「穩定 VM 互連」的需求。因此，讓 NAT 網卡專門負責對外網路，Host-only 網卡專門負責內網通訊，做到職責分離。

server-b 採用Host-only：
`server-b` 作為內部伺服器，並沒有主動連接外部網路的需求。刻意不給它 NAT 網卡，能確保它完全隔離在封閉的內網環境中（無法上網、也不受外部干擾），僅能透過 `dev-a` 進行安全的內部連線與管理。
